...
机器上已经装好的 JDK / Node / Maven(如 `C:\Program Files\nodejs`、`C:\Program Files\Java\jdk-21`)可以直接
「已安装 → ＋添加已有安装」纳入管理:选工具 + 选目录,主进程按清单规则**校验目录并识别版本**,登记后
`current\<tool>` 指向它,于是也能 `/切换版本`。接管**不复制、不移动、不删除**原目录里的任何文件,
不想要了用 [移出登记] 摘掉记录即可(文件原样保留)。
